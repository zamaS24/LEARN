...
# Besoin fonctionnel à ajouter (pas obligatoire) :  
# Variation minSup et Itemset frequents

class App():
    """ Design of the code: 
        Constructor, will create main viewport and each GUI item will be
        initialised, also each item's id is as an object attribute. all items
        variable containers are declared and assigned to None, for readability 
        purposes.
    ...
    def __init__(self):
        ...
        with dpg.font_registry():
            dpg.bind_font(dpg.add_font(
                "resources/fonts/Roboto_Mono/static/RobotoMono-Regular.ttf", 26))
        ...
        with dpg.window(label="Paramètres", no_resize=True, no_close=True,no_open_over_existing_popup=True,
                       width=450, height=450, no_collapse=True, no_move=True ) as self.wdw_main:
            ...
            self.slider_minConfidence = dpg.add_slider_float(label="Min \nConfidence", default_value = 0.5, max_value = 1)
            dpg.add_checkbox(label="Contribution", callback=self.callback_checkbox)
            ...
            self.warningTxt = dpg.add_text("Selectioner un fichier .csv valide!", color=[255,0,0], show=False)



        with dpg.window(label="Règles d'associations, Confiance, lift",show= False, no_resize=False ,no_open_over_existing_popup=True, pos=(450,0) ,width=1200, height=1200 ) as self.wdw_association_rules:
            pass

        with dpg.window(label="Itemsets Fréquents", show = False, no_resize=False, pos=(1650,0) ,width=720, height=1080 ) as self.wdw_frequent_itemsets:
            pass
        
        with dpg.window(label="Statistiques", pos=(0,450), show=False) as self.wdw_plot:

            # create plot 1
            with dpg.plot(no_title=False, no_mouse_pos=True, width=550, height=500):

                # create legend
                dpg.add_plot_legend(label="Fréquence d'apparition de chaque item")

                # create x axis
                dpg.add_plot_axis(dpg.mvXAxis, label="oui", no_gridlines=True, no_tick_marks=True, no_tick_labels=True)
    ...
    def callback_close(self): 
        ...
        if(not self.filePath):
           dpg.configure_item(self.warningTxt, show=True)
            ...
            print(items)
            print(values)
            dpg.add_pie_series(4, 4, 4, values, items, parent=self.plot, angle=90)
    ...
    # Resets the whole side windows, or a specific window is optional
    # def reset(self, item_id=None):
    #     if item_id is None: 
    #         items = [self.wdw_association_rules, self.wdw_frequent_itemsets]
    #         for item in items: 
    #             dpg.delete_item(item)
    #     else:
    #         if not item_id == self.wdw_main:
    #             dpg.delete_item(item_id, children_only=True)
